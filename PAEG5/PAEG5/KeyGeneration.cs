...
namespace PAEG2
{
    public static class KeyGeneration
    {
        ...
        public static List<BigInteger> Encrypt(Keys keys, string message)
        {
            List<BigInteger> result = new List<BigInteger>();
            foreach(var x in message)
            {
                result.Add(BigInteger.ModPow(((int)x), keys.E, keys.N));
            }
            return result;
        }

        public static string Decrypt(Keys keys, List<BigInteger> message)
        {
            StringBuilder sb = new StringBuilder();
            foreach(var x in message)
            {
                sb.Append((char)BigInteger.ModPow(x, keys.D, keys.N));
            }
            return sb.ToString();
        }
    }
